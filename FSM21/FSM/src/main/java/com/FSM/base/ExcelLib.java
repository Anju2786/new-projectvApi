package com.FSM.base;
	...
	public class ExcelLib extends Baseclass {
		...
		public static LinkedHashMap getRequestParameters(String testCaseName, String excelPath, String sheetName) {
			...
			LinkedHashMap<String, String> apiReqData = new LinkedHashMap();
			try {
				...
				for (int i = 0; i <= rows; i++) {
					...
					if (sh.getRow(i).getCell(1).getStringCellValue().equalsIgnoreCase(testCaseName)) {
							...
							for (int j = 1; j < sh.getRow(i).getPhysicalNumberOfCells(); j++) {
							
								
							apiReqData.put(sh.getRow(0).getCell(j).getStringCellValue(),sh.getRow(i).getCell(j).getStringCellValue());
							//apiReqData.put(sh.getRow(1).getCell(j).getStringCellValue(),sh.getRow(i).getCell(j).getStringCellValue());			
